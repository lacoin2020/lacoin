...
#if defined(NDEBUG)
# error "Lacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xda6c6559fc2933cd2c52edff9cc8a74c2ce6f95ecd32c56160ee62971ae8f0d1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lacoin Signed Message:\n";
...
{
    // Lacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Lacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lacoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Lacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x84637912685e06d6c0a560da7c72b32b68c5e6ea93486c052f19dc2d1b1c8232");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Oct/2011 Steve Jobs, Apple’s Visionary, Dies at 56 NYC";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1592861632;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085148681;

        if (fTestNet)
        {
            block.nTime    = 1592861595;
            block.nNonce   = 387140469;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc099644de8fbf7967bd38590335109a25b37208afe790d32b69364244d06ea99"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Lacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LacoinMiner(CWallet *pwallet)
{
    printf("LacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lacoin-miner");
    ...
    try { loop {
        ...
        printf("Running LacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LacoinMiner, pwallet));
}
